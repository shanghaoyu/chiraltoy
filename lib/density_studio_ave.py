...
def smooth_exponential_function(x, y, r0):
    """
    ...
    """
    # Step 1: 构造样条插值器
    spline = CubicSpline(x, y, bc_type='natural')  # 样条插值器

    # Step 2: 计算 r0 处的值和导数
    f_r0 = spline(r0)          # r0 处的值
    f_prime_r0 = spline(r0, 1) # r0 处的一阶导数

    # Step 3: 确定指数函数的衰减速率 B
    # 在 r = r0 处，指数函数的导数为 f_prime_r0
    # g'(r0) = -B * f_r0，因此 B = -f_prime_r0 / f_r0
    B = -f_prime_r0 / f_r0

    # Step 4: 定义分段函数
    def extrapolated_function(r):
        r = np.asarray(r)  # 支持标量或数组输入
        result = np.zeros_like(r, dtype=np.float64)

        # 在 [0, r0] 范围内，使用样条插值
        mask1 = r <= r0
        result[mask1] = spline(r[mask1])

        # 在 (r0, rmax] 范围内，使用指数衰减
        mask2 = r > r0
        result[mask2] = f_r0 * np.exp(-B * (r[mask2] - r0))
...
class one_nucleon_density:
    def __init__(self,nucleusname:str,Anum:int,space:str,forcename:str,hw:int,Nmax:int,genfuncforce='n3loemn500',namen=None,namep=None,namesum=None,smoothrmax=20):
    ...
    def spline_data(self):
        ...
        if self.__radial==True:
            ...
            self.__pweight=0.01
            spline_rhop_sf = smooth_exponential_function(self.__pdata["p"], self.__pdata["rho_re"], self.__pdata["p"][-1])
            spline_rhon_sf = smooth_exponential_function(self.__ndata["p"], self.__ndata["rho_re"], self.__ndata["p"][-1])
            spline_rhosum_sf = smooth_exponential_function(self.__sumdata["p"], self.__sumdata["rho_re"],self.__sumdata["p"][-1])
            # find the max rp of the 
            # index = self.find_last_decreasing_index(self.__pdata["rho_re"])
            self.pp_sf= np.arange(0, self.__rmax, self.__pweight) 
            self.rhop_sf=spline_rhop_sf(self.pp_sf)
            # index = self.find_last_decreasing_index(self.__ndata["rho_re"])
            self.pn_sf= np.arange(0, self.__rmax, self.__pweight) 
            self.rhon_sf=spline_rhon_sf(self.pn_sf)
            # index = self.find_last_decreasing_index(self.__sumdata["rho_re"])
            self.psum_sf= np.arange(0, self.__rmax, self.__pweight) 
            self.rhosum_sf=spline_rhosum_sf(self.psum_sf)        
    ...
    def cal_tilderho_sf_r(self):
        ...
        print(index)
        # calculate the tilderho
        self.xsum_sf=np.arange(0,rmesh[index],self.__pweight)
    ...
    def rhopro_ti_p_link(self,p,klink=[1.8,2.0]):
        # get the general function np
        if self.__cal_gfunpn1==False:
            self.cal_genf_pn1()
            self.__cal_gfunpn1=True
    ...
    def scaling_factor(self,kfermi=1.35,kmax=5,klink=[1.9,2.1],res=False):
